...
int main() {
    ...
    if(new_file.is_open()){
        ...
        while(getline(new_file,sa)){
            ...
            if(nrOfRed <= maxRed && nrOfGreen <= maxGreen && nrOfBlue <= maxBlue)
            {
                cout << "valid game with id:" << gameNrs.back() << endl; 
                validGameIDs=validGameIDs+gameNrs.back();
            }
                cout << "------ validGameIDs: " << validGameIDs << endl;
        }
    }
	return 0;
}    